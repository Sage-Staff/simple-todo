...
app.post("/create-item", async function(req, res){
  try{
    ...
    const item = await db.collection('items').findOne({_id: info.insertedId})
    res.json(item)
...
app.post("/update-item", function(req, res){
  let safeText = sanitizeHTML(req.body.text, {allowedTags: [], allowedAttributes: {}})
  db.collection('items').findOneAndUpdate({_id: new ObjectId(req.body.id)}, {$set: {text: safeText}}).then(function(){
    res.send("Success")
  })
})

app.post("/delete-item", function(req, res){
  db.collection('items').deleteOne({_id: new ObjectId(req.body.id)}).then(function(){
    res.send("Success")
  })
})
